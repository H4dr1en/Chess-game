...
public class Echiquier implements BoardGames {
	...
	public List<PieceIHMs> getPiecesIHM() {
		List<PieceIHMs> listPieces = new ArrayList<PieceIHMs>();
		for(Pieces p : this.jeuBlanc.getPieces()) {
			listPieces.add(new PieceIHM(p));
		}
		for(Pieces p : this.jeuNoir.getPieces()) {
			listPieces.add(new PieceIHM(p));
		}
		return listPieces;		
	}
	...
	public List<Coord> getPossibleMoves(Coord initCoord) {
		...
		for(int j = 0; j < 8 ; j++) {
			for(int i = 0; i < 8 ; i++) {
				if(this.isMoveOk(initCoord.x, initCoord.y, i, j)) {
