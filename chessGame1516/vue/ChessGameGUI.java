...
public class ChessGameGUI extends JFrame implements MouseListener, MouseMotionListener, IObserver{
	...
	private List<Coord> possibleMoves = new ArrayList<Coord>();
	private List<PieceIHM> pieces = new ArrayList<PieceIHM>();

	private Coord pieceToMoveCoords;
	...
	private void drawGrid() {
		...
		for (int i = 0; i<8; i++){
			for (int j = 0; j<8; j++) {
				...
				if(this.isPossibleMove(new Coord(j,i))) {
		...
		// Empty possible moves after having drawn them 
		this.possibleMoves = new ArrayList<Coord>();
		this.revalidate();
	}	
	...
	private void drawPieces() {
		...
		for (Iterator<PieceIHM> iter = this.pieces.iterator(); iter.hasNext(); ) {
			PieceIHM piece = iter.next();
			if(piece.getX() != -1) {
				
				// Draw all pieces but the one moving
				if(this.pieceToMoveCoords == null || 
					(this.pieceToMoveCoords != null &&
						!( this.pieceToMoveCoords.x == piece.getX() && this.pieceToMoveCoords.y == piece.getY() ) 
					)
				) {
					
					jpiece = new JLabel(new ImageIcon(ChessImageProvider.getImageFile(piece.getNamePiece(), piece.getCouleur())));
					jpanel = (JPanel) chessBoardGuiContainer.getComponent(piece.getX() + (piece.getY() * 8));
					jpanel.add(jpiece);
				}
			}			
		}
	...
	private boolean isPossibleMove(Coord coord) {
		for(Coord c : this.possibleMoves) {			
			if(c.equals(coord))
				return true;
		}
		return false;
	...
	public void mousePressed(MouseEvent e){	
		...
		if (c instanceof JLabel) {
			...
			if (this.chessGameControler.isPlayerOK(initCoord))	{
				...
				this.pieceToMoveCoords = translateCoord(e.getX(), e.getY());
				
				// Mise en évidence des cases vers lesquelles 
				// la pièce peut être déplacée
				
				// ToDo
				this.possibleMoves = this.chessGameControler.getPossibleMoves(this.initCoord);
				this.drawGrid();
				this.drawPieces();
